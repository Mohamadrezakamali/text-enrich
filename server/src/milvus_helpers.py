...
class MilvusHelper:
    ...
    def create_collection(self, collection_name, vec_dim=VECTOR_DIMENSION):
        # Create milvus collection if not exists
        try:
            if not self.has_collection(collection_name):
                field1 = FieldSchema(name="id", dtype=DataType.INT64, descrition="int64", is_primary=True,auto_id=True)
                field2 = FieldSchema(name="embedding", dtype=DataType.FLOAT_VECTOR, descrition="float vector", dim=vec_dim, is_primary=False)
    ...
    def insert(self, collection_name, vectors):
        ...
        try:
            self.create_collection(collection_name, vec_dim=len(vectors[0]))
            self.collection = Collection(name=collection_name)
            data = [vectors]
            ...
            # try:
            #     index_params = {"index_type": "IVF_FLAT", "params": {"nlist": 2}, "metric_type": "L2"}
            #     self.collection.create_index('embedding', index_params, index_name="embeddingIndex")
            # except:
            #     pass
    ...
    def load_all(self):
        try:
            for collection_name in self.list_collections():
                self.collection = Collection(name=collection_name).load()
                
        except Exception as e:
            LOGGER.error(f"Failed to load collections into RAM: {e}")
            sys.exit(1)
