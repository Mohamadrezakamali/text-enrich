...
class MySQLHelper():
    ...
    def create_mysql_table(self, table_name):
        ...
        sql = "create table if not exists " + table_name + "(milvus_id TEXT, text TEXT) CHARACTER SET utf8mb4;"
        try:
    ...
    def load_data_to_mysql(self, table_name, data):
        ...
        self.test_connection()
        sql = "insert into " + table_name + " (milvus_id,text) values (%s,%s);"
        try:
    ...
    def search_by_milvus_ids(self, ids, table_name):
        ...
        try:
            ...
            results_text = [res[1] for res in results]
            LOGGER.debug("MYSQL search by milvus id.")
            return results_id, results_text
        except Exception as e:
            LOGGER.error(f"MYSQL ERROR: {e } with sql: {sql}")
            sys.exit(1)
